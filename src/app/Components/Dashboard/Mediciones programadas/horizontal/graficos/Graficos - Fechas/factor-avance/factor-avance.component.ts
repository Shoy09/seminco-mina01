...
export class FactorAvanceComponent implements OnChanges {
  ...
  const agrupados = filtrados.reduce((acc, d) => {
    ...
    if (!acc[labor]) {
      acc[labor] = { kg_explosivos: 0, avance_programado: 0, count: 0 };
    }
    acc[labor].kg_explosivos += d.kg_explosivos || 0;
    acc[labor].avance_programado += d.avance_programado || 0;
    acc[labor].count += 1;
    return acc;
  }, {} as Record<string, { kg_explosivos: number, avance_programado: number, count: number }>);

  // 🔹 Convertir a arrays para ApexCharts
  const categories = Object.keys(agrupados);
  const rendimiento = categories.map(labor => {
    const { kg_explosivos, avance_programado } = agrupados[labor];
    return avance_programado > 0 ? kg_explosivos / avance_programado : 0;
  });
  const avance = categories.map(labor => {
    const { avance_programado, count } = agrupados[labor];
    return count > 0 ? (avance_programado / count) : 0;
  });
  ...
  // 🔹 Agregar promedio como categoría final
